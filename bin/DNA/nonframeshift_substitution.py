...
class NonfsSubstitutionGenerator:
    """
    Generate variant peptides from ANNOVAR annotation results for nonframeshift substitutions
    
    This class processes ANNOVAR mutation annotations and generates variant peptide
    sequences specifically for nonframeshift substitution mutations
    """
    ...
    def run(self):
        """Execute the peptide generation workflow for nonframeshift substitutions"""
        try:
            # Read input data
            ...
            # Process nonframeshift substitution mutations
            self.process_substitutions(sample, reuniprot)
            
            # Save final variant sequences
            self.save_var_pro()
            
            logger.info("Nonframeshift substitution variant peptide generation completed successfully")
    ...
    def process_substitutions(self, sample, reuniprot):
        """Process nonframeshift substitution mutations"""
        logger.info("Processing nonframeshift substitution mutations")
        tag = 'aa_sub'  # amino acid substitution
        mutation_type = 'nonframeshift substitution'
        
        # Filter samples with nonframeshift substitution mutations
        ...
        # For cDNA: matches patterns like c.1587_1588delinsCC
        pattern_c = re.compile(r'c\.(\d+_\d+delins[A-Z]+)')
        # For protein: matches patterns like p.V530L (amino acid change)
        pattern_p = re.compile(r'p\.([A-Z]\d+[A-Z])')
    ...
    def find_gene_c_p(self, sample_df, pattern_c, pattern_p):
        ...
        for index, row in sample_df.iterrows():
            try:
                ...
                for s in str_list:
                    ...
                    # Extract protein change for substitution
        ...
        logger.info(f"Extracted {len(result_df)} unique nonframeshift substitution mutations")
    ...
    def generate_variant_seqs(self, result_df, reuniprot, tag):
        """
        Generate variant sequences for nonframeshift substitution mutations
        ...
        try:
            ...
            # Generate altered sequence and also extract substitution boundaries
            alt_df[['alt_seq', 'sub_start', 'sub_end']] = alt_df.apply(
                lambda row: pd.Series(self.alter_seq_sub_with_boundaries(row['sequence'], row['protein'])), 
                axis=1)
            ...
            alt_df['pos'] = alt_df['protein'].apply(self.get_sub_pos)
            alt_df['AAc'] = alt_df.apply(lambda row: self.get_aac_sub(row['protein'], row['alt_seq']), axis=1)
    ...
    def alter_seq_sub_with_boundaries(self, sequence, protein):
        """
        Alter sequence for nonframeshift substitutions and return substitution boundaries
        
        Args:
            sequence: Original protein sequence
            protein: Protein change notation (e.g., 'V530L')
            
        Returns:
            tuple: (altered_sequence, substitution_start, substitution_end)
        """
        try:
            # Handle full protein format (p.V530L)
            if protein.startswith('p.'):
                protein = protein[2:]  # Remove 'p.' prefix
                
            # Parse the protein substitution format: X###Y
            # Where X is the original amino acid, ### is the position, Y is the new amino acid
            ...
            # Validate positions
            if pos > len(sequence):
                logger.warning(f"Position out of range for substitution {protein}: sequence length {len(sequence)}")
                return "", 0, 0
            
            # Validate reference amino acid
            if sequence[pos-1] != original_aa:
                logger.warning(f"Reference AA mismatch for {protein}: expected {original_aa}, found {sequence[pos-1]}")
                logger.debug(f"Position: {pos}")
                logger.debug(f"Sequence excerpt: {sequence[max(0, pos-5):min(len(sequence), pos+5)]}")
                return "", 0, 0
            
            # Perform substitution
            altered_seq = sequence[:pos-1] + new_aa + sequence[pos:]
            
            return altered_seq, pos, pos
                
        except Exception as e:
            logger.warning(f"Error altering sequence for substitution {protein}: {str(e)}")
            import traceback
            logger.debug(traceback.format_exc())
        
        return "", 0, 0
    
    def get_sub_pos(self, protein):
        """
        Get position for substitution mutations
        ...
        try:
            ...
            # Extract position (all digits between letters)
            return int(''.join(c for c in protein if c.isdigit()))
        except Exception as e:
            logger.warning(f"Error extracting position for substitution {protein}: {str(e)}")
            return 0
    
    def get_aac_sub(self, protein, alt_seq):
        """
        Get amino acid changes for substitutions
        ...
        try:
            ...
            # Extract original and new amino acids
            original_aa = protein[0]
            new_aa = protein[-1]
            
            return f"{original_aa}>{new_aa}"
        except Exception as e:
            logger.warning(f"Error extracting AA change for substitution {protein}: {str(e)}")
            return "sub_change"
    ...
    def get_junction_peptides(self, original_seq, alt_seq, sub_start, sub_end, min_len, max_len):
        """
        Generate peptides that span the junction of a substitution
        
        Args:
            original_seq: Original protein sequence
            alt_seq: Altered protein sequence with substitution
            sub_start: Start position of substitution (1-based)
            sub_end: End position of substitution (1-based)
        ...
        # We have two junctions to consider for substitutions:
        # 1. The junction at the start of the substitution (between sub_start-1 and sub_start)
        # 2. The junction at the end of the substitution (between sub_end and sub_end+1)
        
        junction_positions = [sub_start-1, sub_end]
    ...
    def save_var_pro(self):
        ...
        # Load nonframeshift substitution mutation data
        try:
        ...
        if found_mutations and not alt_df.empty:
            ...
            # Save to file
            fasta_path = os.path.join(self.out_file, 'AASUB_Varsequence.fasta')
            ...
            summary_path = os.path.join(self.out_file, 'AASUB_gene_summary.csv')
    ...
    def create_junction_peps(self, alt_df, min_len, max_len):
        """
        Create peptides specifically spanning substitution junctions
        ...
        for _, row in alt_df.iterrows():
            try:
                ...
                junction_peps = self.get_junction_peptides(
                    original_seq, alt_seq, sub_start, sub_end, min_len, max_len)
...
def main():
    """Parse arguments and run the nonframeshift substitution peptide generator"""
    parser = argparse.ArgumentParser(description='Generate variant peptides for nonframeshift substitutions from ANNOVAR annotations')
    ...
    logger.info(f"Starting nonframeshift substitution variant peptide generation")
    ...
    generator = NonfsSubstitutionGenerator(
