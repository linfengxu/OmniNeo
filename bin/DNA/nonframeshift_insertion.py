...
class NonfsInsertionGenerator:
    """
    Generate variant peptides from ANNOVAR annotation results for nonframeshift insertions
    
    This class processes ANNOVAR mutation annotations and generates variant peptide
    sequences specifically for nonframeshift insertion mutations
    """
    ...
    def run(self):
        """Execute the peptide generation workflow for nonframeshift insertions"""
        try:
            # Read input data
            ...
            # Process nonframeshift insertion mutations
            self.process_insertions(sample, reuniprot)
            
            # Save final variant sequences
            self.save_var_pro()
            
            logger.info("Nonframeshift insertion variant peptide generation completed successfully")
    ...
    def process_insertions(self, sample, reuniprot):
        """Process nonframeshift insertion mutations"""
        logger.info("Processing nonframeshift insertion mutations")
        tag = 'nfs_ins'
        mutation_type = 'nonframeshift insertion'
        
        # Filter samples with nonframeshift insertion mutations
        ...
        pattern_c = re.compile(r'c\.(\d+_\d+ins[A-Z]+|\d+ins[A-Z]+)')
        # Match patterns like p.V600_K601insEA or p.V600insE
        pattern_p = re.compile(r'p\.([A-Z]\d+(?:_[A-Z]\d+)?ins[A-Z]+)')
    ...
    def find_gene_c_p(self, sample_df, pattern_c, pattern_p):
        ...
        for index, row in sample_df.iterrows():
            try:
                ...
                for s in str_list:
                    ...
                    # Extract protein change for insertion
        ...
        logger.info(f"Extracted {len(result_df)} unique nonframeshift insertion mutations")
    ...
    def generate_variant_seqs(self, result_df, reuniprot, tag):
        """
        Generate variant sequences for nonframeshift insertion mutations
        
        Args:
            result_df: DataFrame with mutation information
            reuniprot: Reference protein database
            tag: Mutation type tag (nfs_ins)
        """
        try:
            ...
            # Generate altered sequence and also extract insertion boundaries
            alt_df[['alt_seq', 'ins_start', 'ins_end']] = alt_df.apply(
                lambda row: pd.Series(self.alter_seq_ins_with_boundaries(row['sequence'], row['protein'])), 
                axis=1)
            ...
            alt_df['pos'] = alt_df['protein'].apply(self.get_ins_pos)
            alt_df['AAc'] = alt_df.apply(lambda row: self.get_aac_ins(row['protein'], row['alt_seq']), axis=1)
    ...
    def alter_seq_ins_with_boundaries(self, sequence, protein):
        """
        Alter sequence for nonframeshift insertions and return insertion boundaries
        
        Args:
            sequence: Original protein sequence
            protein: Protein change notation (e.g., 'A123_B124insDE' or 'A123insD')
            
        Returns:
            tuple: (altered_sequence, insertion_start, insertion_end)
        """
        try:
            # Handle full protein format (p.A123insD)
            if protein.startswith('p.'):
                protein = protein[2:]  # Remove 'p.' prefix
                
            # For protein ins format, the pattern is either A123insB or A123_B124insCD
            if '_' in protein:
                # Range insertion (e.g., 'A123_B124insCD')
                parts = protein.split('_')
                start_part = parts[0]
                end_ins_parts = parts[1].split('ins')
                
                if len(end_ins_parts) != 2:
                    logger.warning(f"Invalid insertion format: {protein}")
                    return "", 0, 0
                    
                end_part = end_ins_parts[0]
                inserted_aas = end_ins_parts[1]
                ...
                if start_pos > len(sequence) or end_pos > len(sequence):
                    logger.warning(f"Position out of range for insertion {protein}: sequence length {len(sequence)}")
                    return "", 0, 0
                ...
                # Perform insertion - for range insertion, we insert between end_pos-1 and end_pos
                # This means we're inserting after the end position amino acid
                altered_seq = sequence[:end_pos] + inserted_aas + sequence[end_pos:]
                return altered_seq, end_pos, end_pos + len(inserted_aas)
                
            else:
                # Single position insertion (e.g., 'A123insD')
                ins_parts = protein.split('ins')
                
                if len(ins_parts) != 2:
                    logger.warning(f"Invalid insertion format: {protein}")
                    return "", 0, 0
                    
                pos_part = ins_parts[0]
                inserted_aas = ins_parts[1]
                ...
                # Validate position
                if pos > len(sequence):
                    logger.warning(f"Position {pos} out of range for insertion: sequence length {len(sequence)}")
                    return "", 0, 0
                ...
                # Perform insertion - for single position insertion, we insert after the position
                altered_seq = sequence[:pos] + inserted_aas + sequence[pos:]
                return altered_seq, pos, pos + len(inserted_aas)
                
        except Exception as e:
            logger.warning(f"Error altering sequence for insertion {protein}: {str(e)}")
            import traceback
            logger.debug(traceback.format_exc())
        
        return "", 0, 0
    
    def alter_seq_ins(self, sequence, protein):
        """
        Alter sequence for nonframeshift insertions
        ...
        Returns:
            Altered sequence
        """
        result, _, _ = self.alter_seq_ins_with_boundaries(sequence, protein)
        return result
    
    def get_ins_pos(self, protein):
        """
        Get position for insertion mutations
        ...
        try:
            ...
            if '_' in protein:
                # Range insertion - return end position before 'ins'
                parts = protein.split('_')
                end_part = parts[1].split('ins')[0]
                return int(end_part[1:])
            else:
                # Single position insertion
                pos_part = protein.split('ins')[0]
                return int(pos_part[1:])
        except Exception as e:
            logger.warning(f"Error extracting position for insertion {protein}: {str(e)}")
            return 0
    
    def get_aac_ins(self, protein, alt_seq):
        """
        Get amino acid changes for insertions
        ...
        try:
            ...
            if '_' in protein:
                ...
                end_aa = end_ins_parts[0][0]
                inserted_aas = end_ins_parts[1]
                
                return f"{end_aa}>ins{inserted_aas}"
            else:
                # Single position insertion
                ins_parts = protein.split('ins')
                ...
                return f"{aa}>ins{inserted_aas}"
        except Exception as e:
            logger.warning(f"Error extracting AA change for insertion {protein}: {str(e)}")
            return f"{protein.split('ins')[0][0]}>ins"
    ...
    def get_junction_peptides(self, original_seq, alt_seq, ins_start, ins_end, min_len, max_len):
        """
        Generate peptides that span the junction of an insertion
        
        Args:
            original_seq: Original protein sequence
            alt_seq: Altered protein sequence with insertion
            ins_start: Start position of insertion (1-based)
            ins_end: End position of insertion (1-based)
        ...
        # We have two junctions to consider for insertions:
        # 1. The junction where insertion begins (between ins_start-1 and ins_start)
        # 2. The junction where insertion ends (between ins_end-1 and ins_end)
        
        junction_positions = [ins_start-1, ins_end-1]
        junction_names = ["start", "end"]
        
        for i, junction_pos in enumerate(junction_positions):
            junction_name = junction_names[i]
            ...
            for peptide_len in range(min_len, max_len + 1):
                ...
                for amino_before in range(1, peptide_len):
                    ...
                    # 3. Which junction (start or end) it spans
                    # 4. Whether it's novel
                    # Format: "pos1-pos2|junction-start|novel" or "pos1-pos2|junction-end"
                    start_pos = junction_pos - amino_before + 2  # +2 for 1-based position and make it inclusive
                    end_pos = junction_pos + amino_after
                    title_suffix = f"{start_pos}-{end_pos}|junction-{junction_name}"
                    if is_novel:
                        title_suffix += "|novel"
    ...
    def save_var_pro(self):
        ...
        # Load nonframeshift insertion mutation data
        try:
        ...
        if found_mutations and not alt_df.empty:
            ...
            # Save to file
            fasta_path = os.path.join(self.out_file, 'NFSINS_Varsequence.fasta')
            ...
            summary_path = os.path.join(self.out_file, 'NFSINS_gene_summary.csv')
    ...
    def create_junction_peps(self, alt_df, min_len, max_len):
        """
        Create peptides specifically spanning insertion junctions
        ...
        for _, row in alt_df.iterrows():
            try:
                ...
                alt_seq = row['alt_seq']
                ins_start = row['ins_start']
                ins_end = row['ins_end']
                
                # Get junction peptides
                junction_peps = self.get_junction_peptides(
                    original_seq, alt_seq, ins_start, ins_end, min_len, max_len)
...
def main():
    """Parse arguments and run the nonframeshift insertion peptide generator"""
    parser = argparse.ArgumentParser(description='Generate variant peptides for nonframeshift insertions from ANNOVAR annotations')
    ...
    logger.info(f"Starting nonframeshift insertion variant peptide generation")
    ...
    generator = NonfsInsertionGenerator(
