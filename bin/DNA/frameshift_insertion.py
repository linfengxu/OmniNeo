...
class InsFrameshiftGenerator:
    """
    Generate variant peptides from ANNOVAR annotation results for frameshift insertions
    
    This class processes ANNOVAR mutation annotations and generates variant peptide
    sequences specifically for frameshift insertion mutations
    """
    ...
    def __init__(self, input_file, reference_file, output_dir, transcript_file=None, generate_peptides=True, 
        ...
        Args:
            ...
            transcript_file: Path to transcript sequences in FASTA format (for real translation)
    ...
    def run(self):
        """Execute the peptide generation workflow for frameshift insertions"""
        try:
            # Read input data
            ...
            # Process frameshift insertion mutations
            self.process_insertions(sample, reuniprot)
            
            # Save final variant sequences
            self.save_var_pro()
            
            logger.info("Frameshift insertion variant peptide generation completed successfully")
    ...
    def process_insertions(self, sample, reuniprot):
        """Process frameshift insertion mutations"""
        logger.info("Processing frameshift insertion mutations")
        tag = 'fs_ins'
        mutation_type = 'frameshift insertion'
        
        # Filter samples with insertion mutations
        ...
        pattern_c = re.compile(r'c\.(\d+_\d+|\d+)ins[A-Z]*:')
    ...
    def generate_variant_seqs(self, result_df, reuniprot, tag):
        """
        Generate variant sequences for frameshift insertion mutations
        
        Args:
            result_df: DataFrame with mutation information
            reuniprot: Reference protein database
            tag: Mutation type tag (fs_ins)
        """
        try:
            ...
            # Apply the alter_seq_ins function to each row, with a transcript ID if available
            def alter_with_transcript(row):
                # Store the gene as the current transcript ID for use in alter_seq_ins
                self.current_transcript_id = row['gene']
                return self.alter_seq_ins(row['sequence'], row['protein'])
                
            alt_df['alt_seq'] = alt_df.apply(
    ...
    def alter_seq_ins(self, sequence, protein):
        """
        Alter sequence for frameshift insertions
    ...
    def translate_frameshift(self, transcript_id, aa_pos, term_len):
        ...
        try:
            ...
            # For insertion, we simulate inserting 1bp at the position
            nt_pos = (aa_pos - 1) * 3
            ...
            # Simulate a 1bp insertion at the position (different from deletion)
            # This changes the reading frame
            frameshift_seq = transcript_seq[:nt_pos] + "A" + transcript_seq[nt_pos:]
            # Start translating from the current codon
            frameshift_seq = frameshift_seq[nt_pos - (nt_pos % 3):]
    ...
    def get_ins_pos(self, protein):
        """
        Get position for frameshift insertion mutations
        ...
        try:
            ...
            logger.warning(f"Error extracting position for frameshift insertion {protein}: {str(e)}")
            return 0
    
    def get_aac_ins(self, protein, alt_seq):
        """
        Get amino acid changes for frameshift insertions
        ...
        try:
            ...
            logger.warning(f"Error extracting AA change for frameshift insertion {protein}: {str(e)}")
    ...
    def save_var_pro(self):
        ...
        # Load frameshift insertion mutation data
        try:
        ...
        if found_mutations and not alt_df.empty:
            ...
            # Save to file
            fasta_path = os.path.join(self.out_file, 'FSINS_Varsequence.fasta')
            ...
            summary_path = os.path.join(self.out_file, 'FSINS_gene_summary.csv')
...
def main():
    """Parse arguments and run the frameshift insertion peptide generator"""
    parser = argparse.ArgumentParser(description='Generate variant peptides for frameshift insertions from ANNOVAR annotations')
    ...
    logger.info(f"Starting frameshift insertion variant peptide generation")
    ...
    generator = InsFrameshiftGenerator(
